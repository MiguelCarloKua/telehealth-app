...
import { Menu, X, LayoutDashboard, CalendarDays, Users, Settings, LogOut, Bell, Stethoscope, Info } from 'lucide-react';
...
export default function DoctorLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      if (!doctorId) return;
      try {
        const data = await apiCall(`/notifications?userId=${doctorId}`);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden md:flex items-center gap-4">
                ...
                {isNotificationsOpen && (
                  <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden z-50">
                    <div className="p-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
                      ...
                      <button className="text-xs text-violet-600 dark:text-violet-400 font-medium hover:underline">Mark all as read</button>
                    </div>
                      ...
                      {notifications.length === 0 ? (
                        ...
                        notifications.map((notif) => (
                          <div key={notif._id} className="p-4 border-b border-gray-50 dark:border-gray-750 hover:bg-gray-50 dark:hover:bg-gray-750 transition-colors cursor-pointer flex gap-3">
                            <div className="p-2 rounded-xl flex-shrink-0 h-10 w-10 flex items-center justify-center bg-violet-100 dark:bg-violet-900/30 text-violet-500">
                              <Info size={20} />
                            </div>
                            <div>
              ...
              {/* Dynamic Doctor Avatar Profile */}
              <div className="h-10 w-10 rounded-full flex items-center justify-center font-bold overflow-hidden border border-violet-200 dark:border-violet-700 bg-violet-100 dark:bg-violet-900/50 text-violet-700 dark:text-violet-400">
                {profileImage ? (
            ...
            <div className="md:hidden flex items-center gap-4">
              <button 
                onClick={() => setIsNotificationsOpen(!isNotificationsOpen)}
                className="relative text-gray-600 dark:text-gray-300 p-1"
              >
                <Bell size={24} />
                {notifications.length > 0 && (
            ...
            <div className="px-2 pt-2 pb-3 space-y-1">
              {navLinks.map((link) => (
                <Link 
                  ...
                  className="flex items-center gap-3 px-3 py-3 rounded-lg text-base font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700"
                >
